...
def _resolve_dsn() -> tuple[str, dict]:
    dsn = DATABASE_URL
    kwargs: dict = {}
    if not dsn:
        raise RuntimeError("DATABASE_URL not configured in config.py.")
    log.info(f"DB connecting via: {dsn.split('@')[-1].split('/')[0] if '@' in dsn else dsn[:30]}")
...
class Database:
    ...
    async def connect(self):
        try:
            dsn, kwargs = _resolve_dsn()
            # Parse DSN to extract components for manual connection
            if "supabase.co" in dsn:
                import urllib.parse
                ...
                connection_kwargs = {
                    ...
                    "max_size": 8,
                    "ssl": kwargs.get("ssl"),
                    "statement_cache_size": 0  # Disable for Supabase pooler
                }
                self.pool = await asyncpg.create_pool(**connection_kwargs)
            else:
                kwargs["statement_cache_size"] = 0
                self.pool = await asyncpg.create_pool(dsn, min_size=2, max_size=8, **kwargs)
            log.info("Connected to database.")
        except Exception as e:
            log.warning(f"Failed to connect to remote database: {e}")
            # Create local SQLite fallback
            import aiosqlite
            self.local_db_path = "domegle_local.db"
            self.pool = None
            log.info("Using local SQLite database as fallback")
    ...
    async def init_tables(self):
        ...
        elif hasattr(self, 'local_db_path'):
            import aiosqlite
            async with aiosqlite.connect(self.local_db_path) as conn:
                await self._create_sqlite_tables(conn)
        else:
            log.warning("No database connection available for table initialization")
    ...
    async def _create_sqlite_tables(self, conn):
        await conn.executescript("""
            ...
            CREATE TABLE IF NOT EXISTS friends (
                ...
                user_id    INTEGER REFERENCES users(discord_id) ON DELETE CASCADE,
                friend_id  INTEGER REFERENCES users(discord_id) ON DELETE CASCADE,
                status     TEXT DEFAULT 'pending',
                created_at DATETIME DEFAULT CURRENT_TIMESTAMP,
                UNIQUE(user_id, friend_id)
            );
            CREATE TABLE IF NOT EXISTS blocks (
                blocker_id INTEGER REFERENCES users(discord_id) ON DELETE CASCADE,
                blocked_id INTEGER REFERENCES users(discord_id) ON DELETE CASCADE,
                created_at DATETIME DEFAULT CURRENT_TIMESTAMP,
                PRIMARY KEY(blocker_id, blocked_id)
            );
            CREATE TABLE IF NOT EXISTS reports (
                id          INTEGER PRIMARY KEY AUTOINCREMENT,
                reporter_id INTEGER REFERENCES users(discord_id) ON DELETE CASCADE,
                reported_id INTEGER REFERENCES users(discord_id) ON DELETE CASCADE,
                reason      TEXT,
                created_at  DATETIME DEFAULT CURRENT_TIMESTAMP
            );
            CREATE TABLE IF NOT EXISTS stats (
            ...
            INSERT OR IGNORE INTO stats (date) VALUES (date('now'));
        """)
    ...
    async def get_user(self, discord_id: int) -> Optional[dict]:
        if self.pool:
            async with self.pool.acquire() as conn:
                record = await conn.fetchrow("SELECT * FROM users WHERE discord_id = $1", discord_id)
    ...
    async def get_user_by_username(self, username: str) -> Optional[dict]:
        if self.pool:
            async with self.pool.acquire() as conn:
                record = await conn.fetchrow("SELECT * FROM users WHERE LOWER(username) = LOWER($1)", username)
                return dict(record) if record else None
        elif hasattr(self, 'local_db_path'):
            import aiosqlite
            async with aiosqlite.connect(self.local_db_path) as conn:
                conn.row_factory = aiosqlite.Row
                cursor = await conn.execute("SELECT * FROM users WHERE LOWER(username) = LOWER(?)", (username,))
                row = await cursor.fetchone()
                return dict(row) if row else None
        return None
    ...
    async def create_user(self, discord_id: int, username: str) -> bool:
        if self.pool:
            ...
            try:
                async with aiosqlite.connect(self.local_db_path) as conn:
                    await conn.execute("INSERT INTO users (discord_id, username) VALUES (?, ?)", discord_id, username)
                    await conn.commit()
                return True
